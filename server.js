...
`);
try{db.exec("ALTER TABLE generations ADD COLUMN media_json TEXT NOT NULL DEFAULT '[]'");}catch{}
...
const upload=multer({
 ...
 fileFilter:(_req,file,cb)=>{
  ...
  cb(ok?null:new Error("UNSUPPORTED_MEDIA_TYPE"),ok);
 }
});
...
app.get("/api/shopify/install",auth,(req,res)=>{
 const url=String(process.env.SHOPIFY_INSTALL_URL||"").trim();
 if(!url)return res.status(503).json({error:"SHOPIFY_INSTALL_URL_NOT_CONFIGURED"});
 try{new URL(url)}catch{return res.status(503).json({error:"SHOPIFY_INSTALL_URL_INVALID"});}
 res.json({url});
});
...
app.post("/api/generate",auth,async(req,res)=>{
 ...
 try{
  ...
  const info=db.prepare(`INSERT INTO generations(user_id,supplier_url,title,description,images_json,media_json) VALUES(?,?,?,?,?,?)`)
   .run(req.user.uid,url,title,description,JSON.stringify(images),"[]");
  res.json({id:info.lastInsertRowid,product:{title,subtitle:ai?.subtitle||"Une expérience pensée autour de votre produit.",description,benefits:ai?.benefits||[],faq:ai?.faq||[],brandName:ai?.brandName||p.brand||"Maison Nova",price:p.price||"",currency:p.currency||"EUR",images,media:[],imagesBlocked:images.length===0}});
...
app.post("/api/edit",auth,async(req,res)=>{
 ...
 try{
  ...
  const media=JSON.parse(g.media_json||"[]");
  ...
  res.json({id:g.id,credits:u.credits-1,product:{title,subtitle:ai.subtitle||"Une expérience pensée autour de votre produit.",description,benefits:ai.benefits||[],faq:ai.faq||[],brandName:ai.brandName||"Maison Nova",price:"",currency:"EUR",images,media,imagesBlocked:images.length===0}});
...
app.post("/api/generations/:id/media",auth,upload.array("files",12),(req,res)=>{
 const g=db.prepare("SELECT * FROM generations WHERE id=? AND user_id=?").get(req.params.id,req.user.uid);
 if(!g)return res.status(404).json({error:"GENERATION_NOT_FOUND"});
 const files=Array.isArray(req.files)?req.files:[];
 if(!files.length)return res.status(400).json({error:"NO_MEDIA"});
 const origin=`${req.protocol}://${req.get("host")}`;
 const existing=JSON.parse(g.media_json||"[]");
 const added=files.map(f=>({
  url:`${origin}/uploads/${encodeURIComponent(f.filename)}`,
  name:f.originalname||f.filename,
  mimeType:f.mimetype,
  type:/^video\//i.test(f.mimetype)?"VIDEO":"IMAGE"
 }));
 const media=[...existing,...added].slice(0,24);
 db.prepare("UPDATE generations SET media_json=? WHERE id=?").run(JSON.stringify(media),g.id);
...
app.post("/api/export",auth,async(req,res)=>{
 ...
 const g=db.prepare("SELECT * FROM generations WHERE id=? AND user_id=?").get(req.body.generationId,u.id);if(!g)return res.status(404).json({error:"GENERATION_NOT_FOUND"});
 const images=JSON.parse(g.images_json||"[]");
 const imported=JSON.parse(g.media_json||"[]");
 try{
  ...
  const media=[...images.slice(0,10).map(src=>({originalSource:src,mediaContentType:"IMAGE"})),...imported.slice(0,14).map(m=>({originalSource:m.url,mediaContentType:m.type==="VIDEO"?"VIDEO":"IMAGE"}))].slice(0,20);
