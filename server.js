...
app.get("/api/shopify/launch",auth,(req,res)=>{
 // Preferred: a Shopify install link/listing configured for the app.
 // It can be a Shopify App Store listing URL or a custom install link generated by Shopify.
 const install=String(process.env.SHOPIFY_INSTALL_URL||"").trim();
 if(install && /^https:\/\/([a-z0-9-]+\.)?shopify\.com\//i.test(install)){
   return res.redirect(302,install);
 }
 // Fallback for a single known store during testing.
 const shop=String(process.env.SHOPIFY_STORE||"").trim().toLowerCase();
 if(/^[a-z0-9][a-z0-9-]*\.myshopify\.com$/.test(shop)){
   const state=crypto.randomBytes(24).toString("hex");
   res.cookie("shopify_state",state,{httpOnly:true,sameSite:"lax",secure:process.env.NODE_ENV==="production",maxAge:600000});
   const redirect=`${req.protocol}://${req.get("host")}/api/shopify/callback`;
   const q=new URLSearchParams({client_id:process.env.SHOPIFY_API_KEY||"",scope:process.env.SHOPIFY_SCOPES||"read_products,write_products",redirect_uri:redirect,state});
   return res.redirect(302,`https://${shop}/admin/oauth/authorize?${q}`);
 }
 return res.status(503).send("SHOPIFY_INSTALL_URL n'est pas configurée sur le serveur.");
});
